...
export function InvoiceModal({
  ...
  const addressLine = identity.address;
  const phone = identity.phone;
  const email = identity.email;
  const footer = identity.footer;
    ...
    <div className="bg-white border-0 shadow-none p-6 sm:p-8 print:p-4 max-w-3xl mx-auto space-y-4 sm:space-y-5 print:space-y-3.5 text-slate-900 font-sans a4-sheet-page">
      ...
      <div className="flex justify-between items-start pb-3.5 border-b-2 border-slate-900 gap-4">
        <div className="space-y-1">
          ...
          {email && <p className="text-xs sm:text-sm text-slate-600 font-mono">{email}</p>}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <span className="font-extrabold text-slate-900 text-sm">
              {invoice.customer?.name || (invoice.customer?.phone ? "—" : "Walk-in Customer")}
            </span>
          </div>
          <div>
          ...
          <div className="w-56 space-y-0.5 text-xs font-mono">
            ...
            {discount > 0 && (
              <div className="flex justify-between text-rose-600 font-bold">
                <span>{invoice.discountLabel ? invoice.discountLabel.toUpperCase() : "DISCOUNT"}:</span>
      ...
      <div className="pt-2 border-t border-slate-200 flex flex-row items-center justify-between gap-4">
        ...
        <div className="text-left text-[10px] text-slate-700 font-medium space-y-1">
          ...
          <div className="pt-0.5">
            ...
            >
                  {SITE_URL} <ExternalLink className="w-3.5 h-3.5 print:hidden text-brand" />
            </a>}
          </div>
        </div>

        {/* Right Column: Google Review QR Code Block */}
        {BUSINESS.reviewUrl && BUSINESS.assets.reviewQr && (<div className="flex flex-row items-center gap-3 shrink-0 bg-slate-50 p-2 rounded-xl border border-slate-200">
  ...
  return (
    <div className="text-xs text-slate-900 space-y-4 font-sans print:font-mono">
      ...
      <div className="text-center space-y-1 pb-3 border-b border-dashed border-slate-300 print:border-black">
        ...
        {email && <p className="text-[11px] font-medium text-slate-600 print:text-black">{email}</p>}
        {vatRegistered && vatNumber && (
