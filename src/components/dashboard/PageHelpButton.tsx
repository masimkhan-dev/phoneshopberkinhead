...
const TRAINING_MODE_KEY = "birkenhead_training_mode";
...
export function PageHelpButton({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const acknowledged = localStorage.getItem(`birkenhead_tip_ack_${pageKey}`);
      if (!acknowledged) {
        // Show compact info toast
        toastInfo(`Tip (${pageTitle}): ${firstTimeTip}`);
        localStorage.setItem(`birkenhead_tip_ack_${pageKey}`, "true");
      }
    } catch {
