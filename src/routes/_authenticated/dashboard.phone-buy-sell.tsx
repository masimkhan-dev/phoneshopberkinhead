...
function PhoneBuySellPage() {
  ...
  async function openDetail(id: string) {
    if (id.startsWith("direct-")) {
      const found = data?.rows.find((r: any) => r.id === id);
      if (found) {
        setDetailUnit(found);
        setDetailOpen(true);
        return;
                  ...
                  return (
                    <tr key={u.id} className="db-tr-hover">
                      <td className="db-td font-mono text-xs text-muted-foreground font-bold">
                        {u.stock_number}
                        {u.is_direct_sale && (
                          <span className="block text-[10px] text-blue-600 font-sans font-medium">
                            Direct Sale
                          </span>
                        )}
                      </td>
                      ...
                      <td className="db-td font-mono text-xs text-neutral-700 tracking-wider select-all font-semibold">
                        {u.imei1}
                      </td>
                      <td className="db-td">
                      ...
                      <td className="db-td text-right font-mono text-xs font-bold text-foreground">
                        {u.purchase_cost_pence != null && u.purchase_cost_pence > 0
                      ...
                      <td className="db-td">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          ...
                          {isSold ? (u.is_direct_sale ? "Sold (Direct)" : "Sold") : "In Stock"}
                        </span>
                      </td>
                      ...
                      <td className="db-td text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            className="p-1.5 text-muted-foreground hover:text-ink rounded-lg hover:bg-muted transition-colors min-h-[34px] min-w-[34px] cursor-pointer"
                            title="View details"
                          >
                          ...
                          {!isSold && (
                            <button
                              ...
                              className="px-2.5 py-1 text-xs font-bold text-[#AC313F] bg-[#AC313F]/10 border border-[#AC313F]/20 rounded-lg hover:bg-[#AC313F]/20 transition-colors cursor-pointer whitespace-nowrap"
                            >
                              Sell Handset
                            </button>
                          )}
