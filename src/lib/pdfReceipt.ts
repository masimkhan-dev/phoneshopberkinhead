...
export async function generateReceiptPDF(
  ...
  const fileName = `${BUSINESS.shortName.replace(/\s+/g, "-")}-Receipt-${invoice.number.replace(/[^a-zA-Z0-9-]/g, "")}.pdf`;
  ...
  container.innerHTML = `
    ...
    <div style="text-align: center; font-size: 9.5px; color: #111; border-top: 1px dashed #000; padding-top: 8px;">
      <div style="font-weight: 700;">${businessInfo.footer || `Thank you for choosing ${businessInfo.name}!`}</div>
